  ...
  import {
    Plus, Settings, X, CheckSquare, ToggleLeft, 
    MessageSquare, BarChart3, ListChecks, Trash2, Upload,
    AlertCircle
  } from 'lucide-react';
  ...
  const CustomAlert = ({ message, type = 'error', onClose }) => {
    ...
    return (
      <div className={`fixed top-4 right-4 p-4 rounded-lg border ${bgColor} ${textColor} ${borderColor} flex items-center gap-2 max-w-md animate-fade-in`}>
        ...
        <nav className="bg-white border-b shadow-sm">
          <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <button 
                className={`px-4 py-2 bg-blue-600 text-white rounded-lg ${loading ? 'opacity-50 cursor-not-allowed' : 'hover:bg-blue-700'}`}
  ...
  className={`p-3 rounded-lg cursor-pointer transition-colors ${
    currentQuestion?.id === question.id || currentQuestion?._id === question._id
