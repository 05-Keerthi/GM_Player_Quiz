...
describe('UserSurveyPlay', () => {
  ...
  beforeEach(() => {
    ...
    useSearchParams.mockReturnValue([new URLSearchParams('?sessionId=test123')]);
  ...
  test('handles socket connection and events properly', async () => {
    ...
    expect(mockSocket.on).toHaveBeenCalledWith('next-survey-question', expect.any(Function));
    expect(mockSocket.on).toHaveBeenCalledWith('timer-sync', expect.any(Function));
  });
  ...
  test('handles guest user properly', () => {
    useSurveySessionContext.mockReturnValue({
  ...
  test('handles session end properly', async () => {
