...
describe("FinalLeaderboard Component", () => {
  ...
  test("displays no participants message when leaderboard is empty", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("No participants in this quiz")
      ).toBeInTheDocument();
    });
  });
  ...
  test("displays user score and rank for regular user view", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("user-score")).toHaveTextContent("90 pts");
      expect(screen.getByTestId("user-rank")).toHaveTextContent("#1");
    });
  });
  ...
  test("displays no score message for user without score", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("no-score-message")).toHaveTextContent(
        "No score recorded"
      );
    });
  });
