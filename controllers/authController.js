...
const { generateAccessToken, generateRefreshToken, verifyToken } = require("../services/authService");
...
const register = async (req, res) => {
  try {
    ...
    if (await User.findOne({ username })) {
      return res.status(400).json({ field: "username", message: "Username is already registered" });
    }
    if (await User.findOne({ email })) {
      return res.status(400).json({ field: "email", message: "Email is already registered" });
    }
    if (await User.findOne({ mobile })) {
      return res.status(400).json({ field: "mobile", message: "Phone number is already registered" });
    }
