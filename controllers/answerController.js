...
const calculateScore = (
  ...
  if (Array.isArray(answer)) {
    const correctCount = answer.filter((ans) =>
      correctAnswers.includes(ans)
    ).length;
    const incorrectCount = answer.filter(
      (ans) => !correctAnswers.includes(ans)
    ).length;
    const pointsPerCorrectOption = Math.floor(
      basePoints / correctAnswers.length
    );
...
exports.submitAnswer = async (req, res) => {
  ...
  const { answer: submittedAnswer, timeTaken } = req.body;
  ...
  try {
    ...
    if (!session) {
      return res
        .status(404)
        .json({ message: "Session not found or not in progress" });
    }
    ...
    if (
      ...
      return res
        .status(400)
        .json({
          message: "Answer must be an array for multiple_select questions",
        });
    } else if (
      ["multiple_choice", "true_false", "open_ended", "poll"].includes(
        question.type
      ) &&
      ...
      return res
        .status(400)
        .json({ message: "Answer must be a string for this question type" });
    }
    ...
    if (!answerType) {
      return res
        .status(400)
        .json({ message: "Unsupported question type or invalid mapping" });
    }

    // Process the answer and determine if it's correct
    let isCorrect = false;
    let pointsAwarded = 0;
    let processedAnswer = submittedAnswer;

    if (answerType === "option") {
      if (Array.isArray(submittedAnswer)) {
        // Convert indices to option text values for multiple select
        ...
        const correctCount = processedAnswer.filter((ans) =>
          correctAnswers.includes(ans)
        ).length;
        const incorrectCount = processedAnswer.filter(
          (ans) => !correctAnswers.includes(ans)
        ).length;
        ...
        // If any answers are correct, mark as correct
        isCorrect = correctCount > 0;
      } else {
        isCorrect = question.options.some(
          (opt) => opt.text === processedAnswer && opt.isCorrect
        );
      ...
      isCorrect = question.correctAnswer.some(
        (correctAnswer) =>
          correctAnswer.toLowerCase() === processedAnswer.toLowerCase()
      );
      pointsAwarded = isCorrect
        ? Math.floor(
            calculateScore(
    ...
    // Save the answer with processed value
    ...
    const options = question.options.map((opt) => ({
      ...
      percentage:
        totalResponses > 0
          ? Math.floor((optionStats[opt.text] / totalResponses) * 100)
          : 0,
    }));
    ...
    // Recalculate ranks
    const allLeaderboardEntries = await Leaderboard.find({
      session: sessionId,
    }).sort({ score: -1 });
...
//     // Check if the user has already answered this question in the session
//     let existingAnswer = await Answer.findOne({ user: userId, session: sessionId, question: questionId });
...
//     // Determine if this is a new answer or an update
//     let populatedAnswer;
//     let pointDifference = 0;

//     if (existingAnswer) {
//       // Update existing answer
//       pointDifference = pointsAwarded - (existingAnswer.pointsAwarded || 0);
//       existingAnswer.answer = answer;
//       existingAnswer.isCorrect = isCorrect;
//       existingAnswer.timeTaken = timeTaken;
//       existingAnswer.pointsAwarded = pointsAwarded;
//       await existingAnswer.save();
//       populatedAnswer = await Answer.findById(existingAnswer._id).populate({
//         path: 'user',
//         select: 'username email mobile role tenantId',
//       });
//     } else {
...
//         pointsAwarded
//       });
//       populatedAnswer = await Answer.findById(newAnswer._id).populate({
//         path: 'user',
//         select: 'username email mobile role tenantId',
//       });
//     }
...
//     if (leaderboardEntry) {
//       leaderboardEntry.score += existingAnswer ? pointDifference : pointsAwarded;
//       await leaderboardEntry.save();
//     } else {
//       await Leaderboard.create({
//         session: sessionId,
...
//   } catch (error) {
//     console.error('Error Details:', error);
//     res.status(500).json({ message: 'Error submitting/updating answer', error });
//   }
// };
...
exports.getSessionAnswers = async (req, res) => {
  ...
  try {
    ...
    const answers = await Answer.find({ session: sessionId }).populate(
      "question user"
    );
  ...
  try {
    ...
    if (!answers || answers.length === 0) {
      return res
        .status(404)
        .json({
          message: "No answers found for this question within the session",
        });
    }
    ...
    res
      .status(500)
      .json({ message: "Error fetching answers for the question", error });
  }
};
