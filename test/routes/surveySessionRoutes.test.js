...
jest.mock('../../controllers/surveySessionController', () => ({
  createSurveySession: jest.fn((req, res) =>
    res.status(201).json({ message: 'Survey session created successfully' })
  ),
  joinSurveySession: jest.fn((req, res) =>
    res.status(200).json({ message: 'Joined survey session successfully' })
  ),
  startSurveySession: jest.fn((req, res) =>
    res.status(200).json({ message: 'Survey session started successfully' })
  ),
  nextSurveyQuestion: jest.fn((req, res) =>
    res.status(200).json({ message: 'Next survey question retrieved successfully' })
  ),
  endSurveySession: jest.fn((req, res) =>
    res.status(200).json({ message: 'Survey session ended successfully' })
  ),
}));

// Import the mocks after defining them
const { auth, optionalAuth, isAdminOrTenantAdmin } = require('../../middlewares/auth');
